...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for university in universities:
            ...
            user = get_user_model().objects.create_user(email=email, username=username, password=username)
            ...
            for organization in organizations:
                username =  (filter(str.isupper, organization.name) + university.acronym).lower()
                email = username + '@' + username + '.com'
                user = get_user_model().objects.create_user(email=email, username=username, password=username)
                host = Host.objects.create(user=user, organization=organization, university=university, administrator=administrator)
                hosts.append(host)
                print host
